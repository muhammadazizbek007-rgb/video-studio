 ...
 * The UI pickers (style preset, camera motion) submit labels, not prompt text,
 * so each label is expanded into film vocabulary before being appended.
 *
 * Pure module by design: it takes plain data and returns strings, so prompt
 * behaviour can be tested without Firebase, Vertex or config.
 */
 ...
 * for the model — these describe the look each preset is meant to produce.
 */
 ...
 * speech, and watermarks or artifacts are never wanted.
 */
 ...
 *
 * Cyrillic stems are matched without \b because JavaScript word boundaries are
 * ASCII-only and would not fire between a Cyrillic letter and a space.
 */
